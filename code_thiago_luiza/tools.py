...
###############################################################################
def coordinates3D(nx, ny, nz, Lx, Ly, Lz, Gnx, Gny, Gnz, GLx, GLy, GLz):
    '''Generate the grid coordinates'''
    dx = GLx / Gnx
    dy = GLy / Gny
    dz = GLz / Gnz
...
def coordinates3D_MPI_v2(Gnx, Gny, Gnz, GLx, GLy, GLz,
    ...
    Gnx, Gny, Gnz : número total de células (global)
    GLx, GLy, GLz : tamanhos físicos do domínio global
    Px, Py, Pz    : número de processos em cada direção
    rank          : rank MPI (converteremos para (px,py,pz))
    """

    # ---------------------------
    # 1. Conversão rank 1D → 3D
    # ---------------------------
    px =  rank % Px
    py = (rank // Px) % Py
    pz =  rank // (Px * Py)
    ...
    # ---------------------------
    # 2. Tamanho do bloco global
    # ---------------------------
    nx = Gnx // Px
    ny = Gny // Py
    nz = Gnz // Pz

    # Ajuste para divisões não inteiras
    if px == Px - 1: nx += Gnx % Px
    if py == Py - 1: ny += Gny % Py
    if pz == Pz - 1: nz += Gnz % Pz
    ...
    # ---------------------------
    # 3. Ghost cells por direção
    # ---------------------------
    gx_left  = 1 if px > 0 else 0
    gx_right = 1 if px < Px - 1 else 0
    gy_left  = 1 if py > 0 else 0
    gy_right = 1 if py < Py - 1 else 0
    gz_left  = 1 if pz > 0 else 0
    gz_right = 1 if pz < Pz - 1 else 0

    nx_local = nx + gx_left + gx_right
    ny_local = ny + gy_left + gy_right
    nz_local = nz + gz_left + gz_right

    # ---------------------------
    # 4. Índices globais
    # ---------------------------
    global_x_start = px * (Gnx // Px)
    global_y_start = py * (Gny // Py)
    global_z_start = pz * (Gnz // Pz)

    # Corrige última partição
    if px == Px - 1:
        global_x_end = Gnx
    else:
        global_x_end = (px + 1) * (Gnx // Px)

    if py == Py - 1:
        global_y_end = Gny
    else:
        global_y_end = (py + 1) * (Gny // Py)

    if pz == Pz - 1:
        global_z_end = Gnz
    else:
        global_z_end = (pz + 1) * (Gnz // Pz)

    # ---------------------------
    # 5. Construir vetores globais
    # ---------------------------
    x_global = (np.arange(Gnx) + 0.5) * dx
    y_global = (np.arange(Gny) + 0.5) * dy
    z_global = (np.arange(Gnz) + 0.5) * dz

    # ---------------------------
    # 6. Criar vetores locais com ghost cells
    # ---------------------------
    x_idx = np.arange(global_x_start - gx_left,
                      global_x_end   + gx_right)
    y_idx = np.arange(global_y_start - gy_left,
                      global_y_end   + gy_right)
    z_idx = np.arange(global_z_start - gz_left,
                      global_z_end   + gz_right)

    coord = np.zeros((nx_local * ny_local * nz_local, 3))
    idx   = np.zeros((nx_local * ny_local * nz_local, 3), int)

    pos = 0
    for k, gz_ in enumerate(z_idx):
        for j, gy_ in enumerate(y_idx):
            for i, gx_ in enumerate(x_idx):
                coord[pos] = [x_global[gx_], y_global[gy_], z_global[gz_]]
                idx[pos]   = [gx_, gy_, gz_]
                pos += 1
